...
BuildRequires:  %{?scl_prefix_maven}mvn(com.fasterxml.jackson.core:jackson-core)
BuildRequires:  %{?scl_prefix_maven}mvn(com.fasterxml.jackson.core:jackson-databind)
BuildRequires:  %{?scl_prefix_maven}mvn(com.sun.mail:javax.mail)
BuildRequires:  %{?scl_prefix_maven}mvn(org.apache:apache:pom:)
BuildRequires:  %{?scl_prefix_maven}mvn(org.apache.commons:commons-compress)
BuildRequires:  %{?scl_prefix_maven}mvn(org.apache.felix:maven-bundle-plugin)
BuildRequires:  %{?scl_prefix_maven}mvn(org.fusesource.jansi:jansi)
BuildRequires:  %{?scl_prefix_maven}mvn(org.slf4j:slf4j-api)
BuildRequires:  %{?scl_prefix_maven}mvn(org.slf4j:slf4j-ext)

BuildRequires:  %{?scl_prefix}mvn(com.lmax:disruptor)
BuildRequires:  %{?scl_prefix}mvn(org.apache.logging:logging-parent:pom:)
...
%pom_disable_module %{pkg_name}-jcl
%pom_disable_module %{pkg_name}-taglib
%pom_disable_module %{pkg_name}-jmx-gui
%pom_disable_module %{pkg_name}-bom
%pom_disable_module %{pkg_name}-nosql
%pom_disable_module %{pkg_name}-web
%pom_disable_module %{pkg_name}-iostreams
%pom_disable_module %{pkg_name}-jul
%pom_disable_module %{pkg_name}-liquibase
%pom_disable_module %{pkg_name}-core-its
...
%changelog
* Fri Jun 23 2017 Michael Simacek <msimacek@redhat.com> - 2.8.2-1.1
